...
#define FUNA_SENSOR_ID		1  //
#define HELITAI_SENSOR_ID 		0  //
#define JUNDA_SENSOR_ID 		4  //
...
// add start by yangjuwei
	if (sensor_id  == DIJIN_SENSOR_ID)
	{
	     if (CHIP_TYPE_GT9F == gtp_chip_type)
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xxF","DiJin",client->addr);
	     else
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xx","DiJin",client->addr);
	}
	else if (sensor_id == FUNA_SENSOR_ID) 
	{
	     if (CHIP_TYPE_GT9F == gtp_chip_type)
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xxF","FuNa",client->addr);
	     else
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xx","FuNa",client->addr);
	}
	else if (sensor_id == HELITAI_SENSOR_ID) 
	{
	     if (CHIP_TYPE_GT9F == gtp_chip_type)
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xxF","HeLiTai",client->addr);
	     else
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xx","HeLiTai",client->addr);
	}
	else if (sensor_id == JUNDA_SENSOR_ID) 
	{
	     if (CHIP_TYPE_GT9F == gtp_chip_type)
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xxF","JunDa",client->addr);
	     else
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,","gt9xx","JunDa",client->addr);
	}
	else
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,sensor id :%d","gt9xx","Unkown",client->addr,sensor_id);

	fix_tp_proc_info(tp_info, len);

//add end by yangjuwei

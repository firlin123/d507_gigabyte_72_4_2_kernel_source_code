...
//+++rgk add for detct i2c ok or not to judge firmware ok or not by anxiang.xiao 20140328
static  int	 flag_i2c_ok = 0;
   ...
   // i2c_write(0x26, &dbbus_tx_data[0], 3);
    ...
    //i2c_read(0x26, &dbbus_rx_data[0], 4);
		...
		i2c_master_send(i2c_client, dbbus_tx_data, 3);
	    	msleep(50);
	   	i2c_master_recv(i2c_client, dbbus_rx_data, 4);

		major_temp = (dbbus_rx_data[1]<<8)+dbbus_rx_data[0];
		if(major_temp & 0xf000 == 0)
			break;
...
static unsigned char CTPM_FW_D507_FN_CT045X_RT01[] = {
	// #include "CT045X_RT01_TwoArea_M602_2138A_C182_C152_20131127.i"
	// #include "CT045X_RT01_TwoArea_M602_2138A_C182_C153_20131206.i"
	// #include "CT045X_RT01_TwoArea_M602_2138A_C182_C154_20131216.i"
  // #include "CT045X_RT01_TwoArea_M602_2138A_C182_C158_20131228.i"
 	// #include "CT045X-RT01_TwoArea_M602_2138A_C189_C159_20130109.i"
 	 //	#include "CT045X_RT01_TwoArea_M602_2138A_C190_C15A_20140219.i"
 	 	#include "CT045X-RT01_TwoArea_M602_2138A_C191_C15B_20140313.i"
                                      ...
                                      struct device_attribute *attr, const char *buf, size_t size)
{
    unsigned char dbbus_tx_data[3];
    unsigned char dbbus_rx_data[4] ;
    unsigned short major=0, minor=0, major_temp;
	int time_count = 0;
...
*/
   // fw_version = kzalloc(sizeof(char)*10, GFP_KERNEL);
...
int msg_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	//+++rgk change  for upgrade firmware directly  by anxiang.xiao 20140328
	if (flag_i2c_ok)
	{
		...
		else  if (tp_ic_major == FN_FW_MAJOR)
		{
			...
			else if (0x50 <= tp_ic_minor && tp_ic_minor < 0x60)
			{
				get_i_file_info(CTPM_FW_D507_FN_CT045X_RT01, sizeof(CTPM_FW_D507_FN_CT045X_RT01), &tp_i_file_major, &tp_i_file_minor);
				i_file_fw_version = tp_i_file_minor;
		...
		//end 
		{
			...
			else  if (tp_ic_major == FN_FW_MAJOR)
			{
				...
				{
					size = sizeof(CTPM_FW_D507_FN_CT045X_RT01);

					while (i < 94 * 1024 &&  i < size)
					*p++ =  CTPM_FW_D507_FN_CT045X_RT01[i++];
				}
			}
	...
	// i file module depened ,here for directly upgrade for FN TP  by anxiang.xiao 
	else
	{
		...
		do
		{
			...
			printk("\n@@@ line = %d ,upgrade finished tp_ic_major = 0x%x, tp_ic_minor = 0x%x @@@", __LINE__, tp_ic_major,tp_ic_minor);
		}while (ic_fw_version != i_file_fw_version && upgrade_count < 1);
	}
...
Truly TP [D507]   [FN_CS040X_RT04][firmware:0x32,major:0x4,minor:0x32]----6\n\n\ 
########################################\n\
